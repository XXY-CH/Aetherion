...
- `doctor` 和 `release evidence` 现在会包含 docs deployment readiness inputs：本地 Markdown 入口、相对链接解析、双语伴读链接，以及显式 `public_docs_deployed=false` evidence。
- `release evidence` 现在包含 schema-aligned `release_manifest_preview`，由当前 evidence、source-evidence hashes、可选 remote observations 和 known gaps 派生。它不是写出的 generated manifest file、signed artifact、package、release publication 或 docs deployment。
...
- 在任何新 action run 之前加入 runtime duplicate/idempotency detector、local rate-limit enforcement 和 replay protection。TUI `run` 现在已有 supervisor handoff 前的 local atomic rate-limit window-slot、duplicate-key reservation 和同 key 同 intent cached replay。
- 只读 ingress audit command，证明没有 real remote surface 绕过 Local Supervisor。

验收：

- 不同 normalized intent 的 duplicate idempotency key 和 over-limit local TUI run window 会在创建新 action run 前被发现；同 key 同 intent 且已 completed 的 TUI run 会 replay cached evidence，不执行 live side effect。
- unauthenticated/unknown local API/browser/IM/mobile input 可以被记录成 observation 或 queued intent，但不能授权 tool 或 side effect。

当前部分状态：

- Contract、`ingress audit` surface，以及 TUI `run` local atomic rate-limit、duplicate-key reservation 和同 intent cached replay 已存在。durable/session/remote idempotency replay、durable/distributed/session/remote rate limiting、auth/session lifecycle、public API listener、browser extension ingress、IM delivery、mobile pairing、connector OAuth ingress 和 cloud worker ingress 仍未完成。
...
- 给剩余 registry family 和 projection 做 deterministic rebuild/parity，尤其是剩余 security projections；Store publisher/install records、child-agent budgets/results 和 surface records 现在已有只读 preview。prompt/model 与 security fixture artifacts 需要 artifact-chain evidence、signing/redaction 与显式 repair 覆盖，而不是 registry projection authority。
...
- `audit security-fixtures` 现在为 content assessment、poisoning signal、acknowledgement、decoy-only honeypot trial 和 detector-only regression fixture events 提供只读 artifact evidence-chain preview；它区分 matched、missing-evidence、invalid-artifact、invalid-run-manifest 和 authority-violation 状态，但不读取 raw content、不执行 honeypot subject、不解除 quarantine、不修改 state、不发 lease，也不把 security artifact 当 authority。
- `ledger-integrity-extension-readiness` 现在为 event signature、redaction manifest、rebuild 和显式 repair 前置条件提供 schema-validated design contract，用于不可逆 migration 前的边界检查；`doctor`、`onboarding check` 和 `release evidence` 可以发现该边界 drift，但不会签名 event、迁移 Ledger state、repair projection、创建 transparency log 或授予 authority。
...
- 剩余 security projection rebuild/parity、event signatures、redaction、显式 repair，以及更强的 prompt/model 与 security artifact signing/redaction/repair 仍未完成。Store、surface 与 child-agent projection repair 也仍然必须显式且 operator-approved，不能由 audit 自动修复。
