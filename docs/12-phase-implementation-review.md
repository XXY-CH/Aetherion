...
- TS client path: `callSupervisorRpc` and `runSupervisorKernelLoop`; supervisor result evidence is accepted only from one non-empty JSON-RPC 2.0 response line whose top-level envelope fields are unique, whose id matches the request, and whose envelope contains exactly one of `result` or a non-empty string `error`.
