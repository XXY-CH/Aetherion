...
- `packages/orchestrator/src/index.ts` adds `assemblePromptPlan`, a pure TypeScript prompt assembly function that accepts a task, source-backed Context Pack, and optional selected-run Ledger event envelopes, then emits ordered prompt sections for system boundary, instruction hierarchy, task, assembly manifest, readiness, citation map, response audit, run evidence, memory context, excluded context, tool policy, capability context, context budget, response format, response contract, planner checklist, and verification checklist.
...
- The assembly manifest summarizes included Ledger event ids, selected Memory Cards, Capability Cards, active permissions, allowed tool request names, artifact refs, excluded memories, context conflicts, forbidden tools, non-authorizing guardrails, and risk flags without reading artifacts or granting authority.
- The readiness section reports missing run evidence as a model-preview blocker, surfaces warnings for absent memory, context conflicts, excluded memory, forbidden tools, unread artifact refs, and context-only permissions, and lists next steps without treating them as runtime status or verification results.
...
- Orchestrator unit tests cover source-backed run evidence and memory sections, Context Pack Capability Cards, Context Pack token budgets, deterministic tool allow/deny lists, non-authorizing authority fields, assembly manifest guardrails/risk flags, Prompt Bundle section/message ordering and hashes, readiness blockers/warnings/next steps, citation maps for run events, Memory Cards, sections, and messages, response audit contracts, response audits for passing and failing outputs, instruction hierarchy, response-format blocks/forbidden claims, system/developer/user message assembly, planner/verifier checklists, empty-task fail-closed behavior, no-tool prompt previews, and answer/patch response-format variants. Memory OS unit tests cover the Context Pack memory ranking and budget trimming that feed those prompt sections.
...
- This is prompt engineering, prompt assembly, and local response-output linting only. The Prompt Bundle, planner/verifier checklist, response format, response audit contract, audit result, and message bundles are static guidance or local checks, not an executable planner or runtime verifier. It does not call an LLM provider, perform model routing, run a planner loop, invoke tools, read raw payload artifacts, write memory, grant permissions, or execute actions.
...
- Governance event payloads point to immutable command artifacts. `audit registries` can now report whether registry entries cite existing Ledger event ids, missing event ids, no event provenance, or malformed entries. `audit replay-records`, `audit memory-records`, `audit capsule-records`, and `audit hibernation-records` add scoped rebuild/parity previews for Replay Records, Memory Candidate/Card/Tombstone projections, Capsule lifecycle projections, and Digital Hibernation sleep/wake projections, but deterministic rebuild/parity tooling for remaining registry families is still not implemented.
