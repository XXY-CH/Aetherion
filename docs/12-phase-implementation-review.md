...
- Governance event payloads point to immutable command artifacts. `audit registries` can now report whether registry entries cite existing Ledger event ids, missing event ids, no event provenance, or malformed entries. `audit replay-records` and `audit memory-records` add scoped rebuild/parity previews for Replay Records plus active Memory Card/Tombstone projections, but deterministic rebuild/parity tooling for other registry families is still not implemented.
