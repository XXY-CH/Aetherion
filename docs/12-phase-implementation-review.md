...
- `packages/orchestrator/src/index.ts` adds `assemblePromptPlan`, a pure TypeScript prompt assembly function that accepts a task, source-backed Context Pack, and optional selected-run Ledger event envelopes, then emits ordered prompt sections for system boundary, instruction hierarchy, task, assembly manifest, readiness, citation map, run evidence, memory context, excluded context, tool policy, capability context, context budget, response format, response contract, planner checklist, and verification checklist.
...
- The response format section defines required evidence, assumptions/conflicts, plan/answer/patch, policy/lease-needs, and verification-evidence blocks plus forbidden claims and completion rules for future model-backed planning. It remains static prompt guidance, not an executable planner.
...
- Orchestrator unit tests cover source-backed run evidence and memory sections, Context Pack Capability Cards, Context Pack token budgets, deterministic tool allow/deny lists, non-authorizing authority fields, assembly manifest guardrails/risk flags, readiness blockers/warnings/next steps, citation maps for run events, Memory Cards, sections, and messages, instruction hierarchy, response-format blocks/forbidden claims, system/developer/user message assembly, planner/verifier checklists, empty-task fail-closed behavior, no-tool prompt previews, and answer/patch response-format variants.
- TUI integration runs a real Rust-supervised kernel run, accepts a Memory Card, assembles a prompt plan for that run, asserts the preview includes run evidence, selected memory, source event ids, event types, Boundary Facts artifact refs, assembly manifest guardrails, readiness, citation map, instruction hierarchy, response format, capability context, context budget, planner checklist, verification checklist, and role-bundled messages, and verifies the Ledger file remains byte-identical with no prompt artifact directory created.
...
- This is prompt engineering and prompt assembly only. The planner/verifier checklist, response format, and message bundles are static guidance, not an executable planner. It does not call an LLM provider, perform model routing, run a planner loop, invoke tools, read raw payload artifacts, write memory, grant permissions, or execute actions.
