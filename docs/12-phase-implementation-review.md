...
- `cargo test`: 26 passing Rust tests.
...
- The Rust stdio RPC parser is dependency-free and intentionally minimal; malformed JSON, duplicate keys, wrong-typed required strings, wrong-typed boolean approval fields, and legacy policy/read/write direct RPCs now fail closed, but it is not a robust general JSON-RPC server.
