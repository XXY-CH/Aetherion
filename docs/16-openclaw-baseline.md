...
### Phase 13 - Context Epoch Baseline + Stale Tool-Registration Fence (P2 durable session runner, first slice)

First slice of the P2 "durable session runner / context epoch" goal: anchor a run to the model-visible context it was admitted under. `context-epoch.ts` adds `computeContextEpoch(systemPrompt, tools)` and `toolRegistryDigest(tools)`, producing an order- and key-order-independent sha256 over the advertised tool surface plus a combined baseline hash over the system prompt. `startAgentLoopState` captures the epoch onto `AgentLoopState.contextEpoch`; `runAgentLoop` records it as a durable `context.epoch.recorded` ledger event at loop start, then at the top of every turn re-digests `config.toolRegistry` and, on any drift from the admitted `tools_hash`, appends `context.epoch.violation` and aborts the turn with an `error` event coded `context_epoch_violation` (stale advertised tool registration can only block, never overwrite run state). Two new event types were registered in `schemas/event.schema.json`. This delivers two of the P2 sub-items ("hash of model-visible context baseline" and "stale advertised tool registration is rejected"); tool-settlement binding and durable input admission remain for follow-up slices. Verification: new `packages/harness-core/test/context-epoch.test.ts`, 5/5 (deterministic epoch, order-independent + content-sensitive tool digest, prompt-sensitive context hash, loop captures+records the epoch, drifted tool surface is rejected without completing the loop); `npm test` 373/373.

### Phase 14 - Tool Settlement Bound To Assistant Message + Tool Call Id (P2 durable session runner, second slice)

Second P2 slice: make every tool call settle exactly once against the assistant message that requested it. `tool-settlement.ts` adds a `ToolSettlementTracker` that `register`s a `(assistantMessageId, toolCallId)` admission and `settle`s it once, rejecting a call that was never registered (`unknown_tool_call`), one settled against a different assistant message than it was admitted under (`assistant_mismatch`), or one already settled (`already_settled`). `startAgentLoopState` admits a tracker onto `AgentLoopState.toolSettlement`; each turn derives `assistantMessageId = asst_<runId>_<depth>`, registers the turn's advertised tool calls, then `processToolCall` claims the settlement at entry — recording a durable `tool.settled` ledger event naming the binding, or, on rejection, pushing a failed tool result + `tool.denied` event and resolving the call without executing it. This stops a duplicated tool_call id (or a replayed/stale result) from being honored twice. A `tool.settled` event type was registered in `schemas/event.schema.json`. Durable input admission remains for the final P2 slice. Verification: new `packages/harness-core/test/tool-settlement.test.ts`, 5/5 (settle-once, unknown rejected, assistant mismatch rejected, rebind on re-registration, loop records the binding for a real tool call); `npm test` 378/378.

### Phase 15 - Durable Input Admission (P2 durable session runner, third slice)

Final core P2 slice: durably admit user inputs so a restarted or resumed run recognizes inputs it already accepted instead of re-executing them. `input-admission.ts` adds `admitInput(workspaceRoot, runId, content, sequence)` which writes a `{ input_id, content_hash, sequence, admitted_at }` record to `.aetherion/admissions/<runId>.json` and is idempotent: re-admitting the same `input_<runId>_<sequence>` id returns the original record with `admitted: false` and does not rewrite the store, so a crash-replay of the same turn cannot double-process it. `runAgentLoop` admits the (context-expanded) input before it enters the conversation and records a durable `input.admitted` ledger event only on first admission. The `input.admitted` event type was registered in `schemas/event.schema.json`. With this, all four P2 sub-items from §6 are delivered (context-baseline hash, stale tool-registration rejection, tool settlement binding, durable input admission). Verification: new `packages/harness-core/test/input-admission.test.ts`, 4/4 (idempotent replay keeps a single record, distinct sequences/runs admitted independently, empty load when nothing admitted, loop durably admits + records its input); `npm test` 382/382.
