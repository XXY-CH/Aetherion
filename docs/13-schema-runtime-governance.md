...
Memory registry reads that assemble downstream context must not treat projections as source truth. `context explain`, `prompt plan`, `prompt audit`, `memory user-model`, and hibernation resume context assembly require Memory Card/Tombstone registry entries to pass the read-only registry provenance reference gate before use. Passing this gate means referenced Ledger event ids exist; it still does not prove deterministic registry rebuild parity. `.aetherion/memory/user-model.json` is a projection-only convenience copy derived from accepted Memory Cards.
