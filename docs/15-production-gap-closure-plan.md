...
- `ledger-integrity-extension-readiness` now provides a schema-validated design contract for event signatures, redaction manifests, rebuild, and explicit repair prerequisites before irreversible migration; `doctor`, `onboarding check`, and `release evidence` can detect drift in that boundary without signing events, migrating Ledger state, repairing projections, creating transparency logs, or granting authority.
