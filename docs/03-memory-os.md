...
- Context assembly applies hard exclusions before ranking: deleted, blocked, secret, and disallowed confidential Memory Cards never compete for prompt budget. Remaining Memory Cards are selected deterministically by confidence, source evidence count, estimated prompt footprint, and id, then trimmed to the Context Pack memory-token budget with overflow listed in `excluded_memories`.
