...
- `ether memory timeline <run_id>` builds an Episodic Timeline from source Ledger events, including failure, recovery, user-correction, skill-candidate, and regression-case signals when those signals are explicit in event summaries. These are review prompts only; they do not create Capability Capsules, tests, or Memory Cards by themselves.
- `ether context explain <run_id>`, `ether prompt plan <run_id> --content <task>`, `ether prompt bind-runtime <run_id> --content <task>`, `ether prompt audit <run_id> --content <task> --path <response-file>`, `ether memory user-model`, and hibernation resume context assembly consume Memory Card/Tombstone registries only after referenced Ledger event ids pass a provenance gate. Weak, missing, or invalid memory registry provenance fails closed.
- Context assembly applies hard exclusions before ranking: deleted, blocked, secret, and disallowed confidential Memory Cards never compete for prompt budget. Remaining Memory Cards are selected deterministically by confidence, source evidence count, estimated prompt footprint, and id, then trimmed to the Context Pack memory-token budget with overflow listed in `excluded_memories`. Memory Card `contradicts` links are surfaced in Context Pack `conflicts` when a selected memory contradicts another selected memory, an excluded memory, or a missing referenced memory.
- `ether prompt plan` is the first local Agent Orchestrator prompt-assembly slice. It renders authority, instruction hierarchy, assembly manifest, readiness, citation map, response-audit contract, run evidence, tool, capability context, context-budget, memory, taint, response-format, response-contract, planner-checklist, and verification-checklist sections from the source-backed Context Pack plus Ledger event envelopes. It also emits a Prompt Bundle that fixes the section order, system/developer/user join strategy, rendered section/message hashes, preview hash, and prompt-engineering rules for model-backed planning. `ether prompt bind-runtime` reuses the same provenance-gated assembly to write only the Agent Runtime Invocation metadata artifact and append a supervisor-authored `agent.runtime.bound` governance event in an independent one-event binding run. That metadata records ids, hashes, context refs, authority gates, tool gateway limits, response-audit requirements, and missing-evidence stages, not rendered prompt text or Memory Card prose. `ether prompt prepare-model-request` reads a previously bound runtime invocation artifact, writes no-tools Agent Model Request metadata, and appends a supervisor-authored `agent.model.requested` governance event in an independent one-event request run. `ether prompt invoke-model` re-renders the same prompt path, checks hashes against the bound request, invokes the configured provider, persists only hash/usage response metadata, and records independent `agent.model.responded` and `agent.response.audit.recorded` evidence runs. `ether prompt audit` remains a local lint command for a workspace-local response file. These commands keep evidence text in the user-context message so it cannot override system or developer constraints. The assembly manifest, Prompt Bundle, Agent Runtime Invocation, Agent Model Request, Agent Model Response, and Agent Response Audit summarize included/excluded context, guardrails, risk flags, source-event taint posture, rendered prompt identity, role-boundary hashes, hashes, checks, and future evidence needs, but they do not grant authority. `prompt plan` and `prompt audit` do not persist prompt artifacts, write runtime invocation artifacts, append Ledger events, or grant authority; `prompt bind-runtime` appends only `agent.runtime.bound`; `prompt prepare-model-request` appends only `agent.model.requested`; `prompt invoke-model` appends only model response/audit evidence and does not request tools, issue leases, read raw payload artifacts, persist prompt artifacts, persist raw model output, or authorize side effects. The readiness summary reports missing evidence, source-event taint that claims authorization, warnings, and next steps for model-preview suitability, but it is not a verification result or runtime status. The citation map records run-event and Memory Card source ids that model outputs must cite for memory-derived claims, but it is not a new source of truth. The response format and response audit contract define required answer/plan/patch blocks, citation checks, and forbidden claims, but they are static prompt guidance and local output linting rather than executable planning or runtime verification. Context budget values are planning limits from the Context Pack, not proof of actual model token usage, and Capability Cards do not grant runtime permissions.
