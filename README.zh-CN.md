...
Ledger Integrity Extension Readiness contract 只记录下一步 integrity 扩展的设计边界：event signatures、redaction manifests、不可逆 Ledger migration、projection repair command、transparency log 和 cloud notary 仍未实现。当前 evidence 仍停在 hash-chain、payload-ref、run-manifest 和只读 audit 层，直到显式 operator-approved migration/repair path 落地。
