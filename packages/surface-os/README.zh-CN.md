...
Store package 安装必须先登记本地 publisher key，再用该 trust anchor 校验签名，并解析本地 replay/sandbox evidence；package code 仍不会执行。每个 package 声明的 `replay_record_id`、`run_id` 和 `source_events` 都必须绑定到本地 Ledger-backed Replay Record evidence；package 自报 boolean 或 registry projection row 不能成为安装 authority。
