...
- Fail closed before `context explain`, `prompt plan`, `memory user-model`, or hibernation resume context assembly when Memory Card/Tombstone registry entries have weak, missing, or invalid Ledger provenance. `.aetherion/memory/user-model.json` is a projection-only convenience file, not a truth source.
- Build non-authorizing prompt plans with `prompt plan <run_id> --content <task>`. The preview uses the same source-backed Context Pack plus Ledger event envelopes, emits authority/instruction-hierarchy/assembly-manifest/readiness/citation-map/run-evidence/tool/capability-context/context-budget/memory/taint/response-format/planner/verifier sections plus a rendered prompt preview and system/developer/user message bundles, calls no model, requests no tool, reads no raw payload artifact, appends no Ledger event, and writes no prompt artifact. Context budget values are planning limits, not runtime model accounting, assembly manifests are audit metadata only, readiness is preview suitability metadata only, citation maps are citation guidance only, response formats are static guidance only, and Capability Cards do not grant permissions.
