...
- Build non-authorizing prompt plans with `prompt plan <run_id> --content <task>`. The preview uses the same source-backed Context Pack plus Ledger event envelopes, emits authority/instruction-hierarchy/run-evidence/tool/capability-context/context-budget/memory/taint/planner/verifier sections plus a rendered prompt preview and system/developer/user message bundles, calls no model, requests no tool, reads no raw payload artifact, appends no Ledger event, and writes no prompt artifact. Context budget values are planning limits, not runtime model accounting, and Capability Cards do not grant permissions.
