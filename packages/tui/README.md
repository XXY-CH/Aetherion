...
- Print a read-only local ingress readiness report with `ingress audit`. It checks envelope, auth-state, rate-limit, idempotency, and policy-handoff requirements while keeping duplicate detection, rate-limit enforcement, durable auth/session lifecycle, public API listeners, browser extension ingress, IM delivery, mobile pairing, connector OAuth ingress, and cloud worker ingress unimplemented.
