...
- Build non-authorizing prompt plans with `prompt plan <run_id> --content <task>`. The preview uses the same source-backed Context Pack plus Ledger event envelopes, emits authority/run-evidence/tool/context-budget/memory/taint/planner/verifier sections plus a rendered prompt preview, calls no model, requests no tool, reads no raw payload artifact, appends no Ledger event, and writes no prompt artifact. Context budget values are planning limits, not runtime model accounting.
