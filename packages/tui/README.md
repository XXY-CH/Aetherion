...
For Ledger `payload_ref`s, Ether resolves the local artifact paths it can prove today, including Boundary Facts, Consent Records, Replay Records, Memory lifecycle snapshots, Security scan/ack/trial/fixture snapshots, Capsule lifecycle snapshots, and generic `.aetherion/artifacts/<command>/<topic>/<id>.json` artifacts. Boundary Facts, Consent Records, Replay Records, Memory Candidate/Card/Tombstone snapshots, Content Assessment, Poisoning Signal, Honeypot Trial, Poisoning Regression Fixture, and Capsule draft/test/publish snapshots are schema-checked after JSON parse; Capsule rollback snapshots and other generic artifacts are reported as `schema_status=not_checked`. Non-local or unsupported reference shapes are reported as `unresolved`; missing or unparsable local files are reported without writing repair artifacts.
