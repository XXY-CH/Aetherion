...
- Print a read-only release evidence report with `release evidence`. It combines git head/dirty state, configured CI/action-runtime/platform-smoke evidence, optional operator-supplied CI/CodeQL snapshots, dependency lockfiles, governance/docs checks, Local Ingress Readiness, Model Provider Readiness, Vault Policy Binding, Supervisor Lifecycle Readiness, Supervisor Lifecycle Command fail-closed evidence, Supervisor Socket Auth Boundary evidence, metadata-only Vault Reference readiness, the V1 Core Profile, `doctor`, `security audit`, workspace runtime state, source-document links, and remaining release gaps without packaging, signing, publishing, deploying docs, starting or stopping daemons, repairing stale locks, starting ingress listeners, accepting remote connections, replaying cached idempotent results, running durable/distributed/session/remote rate limiting, issuing sessions, resolving vault secrets, persisting secrets, implementing OAuth, creating connector grants, or mutating `.aetherion`. Use `release remote-evidence` when an operator needs a stdout-only GitHub Actions/CodeQL snapshot to review and pass back through `release evidence --remote-evidence`.
