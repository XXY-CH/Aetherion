...
- Reserve a hash-only local ingress rate-limit slot and idempotency key before `run` reaches the Rust supervisor; repeated `--idempotency-key <key>` values with different normalized intent or a full local rate-limit window fail closed before any new tool request, lease, or action run. Repeated keys with the same normalized intent and a completed source manifest return cached replay evidence without a new manifest, Ledger event, policy decision, lease, or file action.
...
- Print read-only Rust supervisor workspace status with `supervisor status`, including transport, `daemon_running=false`, runtime paths, Ledger hash-chain validity, event count, head pointers, runtime-lock fields, runtime-lock owner process status, and stale-lock detection. `supervisor preflight` derives a lifecycle state and operator next step from the same status evidence. `supervisor start`, `supervisor stop`, and `supervisor recover-stale-lock` are explicit command surfaces that validate a Supervisor Lifecycle Command report and fail closed with `unsupported_fail_closed`; they do not start/stop daemons, kill processes, repair locks, mutate the Ledger, write artifacts, issue sessions or leases, resolve vault secrets, or authorize tools. These are daemon-readiness and unsupported-command previews, not a background service or lock-repair implementation; the foreground Unix socket can also bind one workspace and write a runtime lock while it is live. The Supervisor Lifecycle Readiness, Supervisor Lifecycle Command, and Supervisor Socket Auth Boundary contracts make those boundaries release-checkable.
- Print a read-only from-source onboarding preflight with `onboarding check`. It checks local toolchain availability, repo scripts, lockfiles, CI/governance/docs evidence, Supervisor Lifecycle Readiness, Supervisor Lifecycle Command evidence, Supervisor Socket Auth Boundary evidence, Ledger Integrity Extension Readiness, workspace runtime state, the V1 Core Profile, and next-step commands without installing dependencies, running long verification, starting daemons, repairing state, signing or migrating Ledger events, writing artifacts, appending events, issuing sessions or leases, or initializing `.aetherion`.
- Print a read-only production-readiness report with `doctor`. It checks repo governance/docs/CI/schema/dependency/platform-smoke baselines, docs deployment readiness inputs, the Local Ingress Readiness contract, the Model Provider Readiness contract, the Vault Policy Binding contract, the Supervisor Lifecycle Readiness contract, the Supervisor Lifecycle Command contract, the Supervisor Socket Auth Boundary contract, the Ledger Integrity Extension Readiness contract, the Adapter Gate Readiness contract, the metadata-only Vault Reference contract, and workspace identity/Ledger/run-manifest invariants without initializing workspaces, repairing state, signing or migrating Ledger events, deploying docs, writing artifacts, appending events, issuing leases, resolving secrets, persisting secrets, executing adapters, implementing OAuth, creating connector grants, or calling providers.
- Print a read-only local ingress readiness report with `ingress audit`. It checks envelope, auth-state, rate-limit, idempotency, and policy-handoff requirements, including the TUI run local atomic rate-limit and idempotency reservation/completion evidence, while keeping durable/session/remote idempotency replay, durable/distributed/session/remote rate limiting, durable auth/session lifecycle, public API listeners, browser extension ingress, IM delivery, mobile pairing, connector OAuth ingress, and cloud worker ingress unimplemented.
...
- Print a read-only release evidence report with `release evidence`. It combines git head/dirty state, configured CI/action-runtime/platform-smoke evidence, optional operator-supplied CI/CodeQL snapshots, dependency lockfiles, governance/docs checks, docs deployment readiness inputs, Local Ingress Readiness, Model Provider Readiness, Vault Policy Binding, Supervisor Lifecycle Readiness, Supervisor Lifecycle Command fail-closed evidence, Supervisor Socket Auth Boundary evidence, Ledger Integrity Extension Readiness, Adapter Gate Readiness, metadata-only Vault Reference readiness, the V1 Core Profile, a schema-aligned `release_manifest_preview`, `doctor`, `security audit`, workspace runtime state, source-document links, and remaining release gaps without packaging, signing, publishing, deploying docs, starting or stopping daemons, repairing stale locks, signing or migrating Ledger events, running redaction tooling, running projection repair commands, starting ingress listeners, accepting remote connections, replaying cached idempotent results, running durable/distributed/session/remote rate limiting, issuing sessions, resolving vault secrets, persisting secrets, executing adapters, implementing OAuth, creating connector grants, or mutating `.aetherion`. The manifest preview is stdout-only derived evidence, not a generated signed manifest or package artifact. The docs readiness evidence checks local Markdown entrypoints and relative links while keeping `public_docs_deployed=false`. Use `release remote-evidence` when an operator needs a stdout-only GitHub Actions/CodeQL snapshot to review and pass back through `release evidence --remote-evidence`.
...
npm run ether -- release remote-evidence --workspace . [--branch main]
```
