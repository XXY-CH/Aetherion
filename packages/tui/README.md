...
- Build non-authorizing prompt plans with `prompt plan <run_id> --content <task>`. The preview uses the same source-backed Context Pack, emits authority/tool/memory/taint sections plus a rendered prompt preview, calls no model, requests no tool, appends no Ledger event, and writes no prompt artifact.
