...
- `supervisor status` 和 `supervisor preflight` 输出只读 lifecycle/status evidence，包括 `daemon_running=false`、runtime-lock fields、owner process status 和 stale-lock detection。`supervisor start`、`supervisor stop` 和 `supervisor recover-stale-lock` 是显式命令表面，但只会校验 Supervisor Lifecycle Command report 并以 `unsupported_fail_closed` fail closed；它们不启动/停止 daemon、不 kill process、不修 lock、不修改 Ledger、不写 artifact、不发 session/lease、不解析 vault secret，也不授权 tool。Supervisor Lifecycle Readiness、Supervisor Lifecycle Command 与 Supervisor Socket Auth Boundary contract 将这些边界纳入 release-checkable evidence；它不是 production daemon、lock repair implementation 或 identity system。
- `onboarding check` 输出只读 from-source onboarding preflight，检查本机 toolchain、repo scripts、lockfiles、CI/governance/docs evidence、Supervisor Lifecycle Readiness、Supervisor Lifecycle Command evidence、Supervisor Socket Auth Boundary evidence、Ledger Integrity Extension Readiness、workspace runtime state、V1 Core Profile 和下一步命令；它不安装 dependency、不运行长 verification、不启动 daemon、不修复 state、不签名或迁移 Ledger events、不写 artifact、不追加 event、不发 session 或 lease，也不初始化 `.aetherion`。
- `doctor` 输出只读生产就绪报告，检查 repo governance/docs/CI/schema/dependency/platform-smoke baseline、docs deployment readiness inputs、Local Ingress Readiness contract、Model Provider Readiness contract、Vault Policy Binding contract、Supervisor Lifecycle Readiness contract、Supervisor Lifecycle Command contract、Supervisor Socket Auth Boundary contract、Ledger Integrity Extension Readiness contract、Adapter Gate Readiness contract、metadata-only Vault Reference contract，以及 workspace identity、Ledger hash chain、run manifest 状态；它不初始化 workspace、不修复 state、不签名或迁移 Ledger events、不部署 docs、不追加 Ledger、不写 artifact、不发 lease、不解析 secret、不持久化 secret、不执行 adapter、不实现 OAuth、不创建 connector grant、不调用 provider。
...
- `release evidence` 输出只读 release evidence 报告，汇总 git head/dirty、已配置 CI/action-runtime/platform-smoke evidence、可选 operator-supplied CI/CodeQL 快照、dependency lockfiles、governance/docs checks、docs deployment readiness inputs、Local Ingress Readiness、Model Provider Readiness、Vault Policy Binding、Supervisor Lifecycle Readiness、Supervisor Lifecycle Command fail-closed evidence、Supervisor Socket Auth Boundary evidence、Ledger Integrity Extension Readiness、Adapter Gate Readiness、metadata-only Vault Reference readiness、V1 Core Profile、schema-aligned `release_manifest_preview`、`doctor`、`security audit`、workspace runtime state、source-document links 和剩余 release gaps；它不打包、不签名、不发布、不部署 docs、不启动/停止 daemon、不修复 stale lock、不签名或迁移 Ledger events、不运行 redaction tooling、不运行 projection repair command、不启动 ingress listener、不接受 remote connection、不 replay cached idempotent result、不执行 durable/distributed/session/remote rate limiting、不发 session、不解析 vault secret、不持久化 secret、不执行 adapter、不实现 OAuth、不创建 connector grant，也不修改 `.aetherion`。manifest preview 是 stdout-only derived evidence，不是 generated signed manifest 或 package artifact。docs readiness evidence 只检查本地 Markdown 入口和相对链接，并保持 `public_docs_deployed=false`。当 operator 需要 GitHub Actions/CodeQL stdout-only 快照时，可用 `release remote-evidence` 生成，review 后再传给 `release evidence --remote-evidence`。
