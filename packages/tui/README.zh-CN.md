...
- `run` 在进入 Rust supervisor 前会先预留 hash-only local ingress idempotency key；重复的 `--idempotency-key <key>` 会在任何新 tool request、lease 或 action run 前 fail closed。
