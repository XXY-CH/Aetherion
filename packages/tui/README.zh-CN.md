...
- `onboarding check` 输出只读 from-source onboarding preflight，检查本机 toolchain、repo scripts、lockfiles、CI/governance/docs evidence、Supervisor Lifecycle Readiness、Supervisor Lifecycle Command evidence、Supervisor Socket Auth Boundary evidence、workspace runtime state、V1 Core Profile 和下一步命令；它不安装 dependency、不运行长 verification、不启动 daemon、不修复 state、不写 artifact、不追加 event、不发 session 或 lease，也不初始化 `.aetherion`。
- `doctor` 输出只读生产就绪报告，检查 repo governance/docs/CI/schema/dependency/platform-smoke baseline、docs deployment readiness inputs、Local Ingress Readiness contract、Model Provider Readiness contract、Vault Policy Binding contract、Supervisor Lifecycle Readiness contract、Supervisor Lifecycle Command contract、Supervisor Socket Auth Boundary contract、metadata-only Vault Reference contract，以及 workspace identity、Ledger hash chain、run manifest 状态；它不初始化 workspace、不修复 state、不部署 docs、不追加 Ledger、不写 artifact、不发 lease、不解析 secret、不持久化 secret、不实现 OAuth、不创建 connector grant、不调用 provider。
