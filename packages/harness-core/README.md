...
- Audit Event Ledger `payload_ref` values without mutating runtime state: `auditLedgerPayloadRefs` resolves known local `artifact://` references for Boundary Facts, Consent Records, Replay Records, Capsule lifecycle snapshots, and generic Ether artifacts, validates known contract artifacts against their existing schemas, then reports resolved, missing, invalid JSON, unresolved, schema-valid, schema-invalid, or not-checked references.
