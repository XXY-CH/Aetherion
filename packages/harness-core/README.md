...
- Validate the Local Ingress Readiness, Local Ingress Rate Limit Reservation, Local Ingress Idempotency Reservation, Local Ingress Idempotency Completion, metadata-only Vault Reference, Vault Policy Binding, Model Provider Readiness, Supervisor Lifecycle Readiness, and Supervisor Lifecycle Command contracts, rejecting remote-surface ingress overclaims, unauthenticated authority, rate-limit authority/session/queue overclaims, idempotency authority reuse, raw rate-limit or idempotency key/intent persistence, late rate-limit enforcement, late duplicate detection, mismatched or live idempotency replay claims, raw external payload storage, raw secret, secret-resolution, egress, raw prompt/model payload, OAuth-flow, token-refresh, connector-grant, provider tool-call, model-output authority, production-daemon, stale-lock repair, socket-auth authority, vault-backend, supervisor lifecycle side effects, unsupported-command authority, and supervisor lease-authority claims in schema tests.
