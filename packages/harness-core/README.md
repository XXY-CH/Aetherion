...
- Create and validate Agent Runtime Invocation, Agent Model Request, and Agent Model Response metadata artifacts through local workspace helpers. These helpers persist only schema-valid ids, refs, hashes, gates, usage, and audit state; they do not call providers, persist raw prompt/model text, resolve credentials, request tools, issue leases, or grant authority.
- Audit Event Ledger `payload_ref` values for future `agent.model.requested` and `agent.model.responded` events by resolving `artifact://agent/model-request/<id>` and `artifact://agent/model-response/<id>` and validating them against the request/response metadata schemas.
