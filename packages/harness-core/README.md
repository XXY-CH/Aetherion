...
- Validate the Local Ingress Readiness, Local Ingress Rate Limit Reservation, Local Ingress Idempotency Reservation, Local Ingress Idempotency Completion, metadata-only Vault Reference, Vault Policy Binding, Model Provider Readiness, Supervisor Lifecycle Readiness, Supervisor Lifecycle Command, Supervisor Socket Auth Boundary, Ledger Integrity Extension Readiness, and Adapter Gate Readiness contracts, rejecting remote-surface ingress overclaims, unauthenticated authority, rate-limit authority/session/queue overclaims, idempotency authority reuse, raw rate-limit or idempotency key/intent persistence, late rate-limit enforcement, late duplicate detection, mismatched or live idempotency replay claims, raw external payload storage, raw secret, secret-resolution, egress, raw prompt/model payload, OAuth-flow, token-refresh, connector-grant, provider tool-call, model-output authority, production-daemon, stale-lock repair, socket-auth authority, socket-token persistence, remote socket clients, vault-backed socket-token storage, session/lease issuance by socket token, vault-backend, supervisor lifecycle side effects, unsupported-command authority, supervisor lease-authority, runtime event-signature, redaction-tooling, automatic repair, projection authority, cloud-notary, Ledger migration, real adapter execution, adapter-issued leases, live side-effect replay, supervisor bypass, package execution, and generated-package trust-root claims in schema tests.
...
- Resolve no-tools live model providers behind the existing hash-only response boundary: `openai_responses`, `openai_chat_completions`, `anthropic`, and `gemini`. `openai_chat_completions` is the supported OpenAI completion-style surface; legacy OpenAI `/v1/completions` is not implemented. Provider credentials are read only in memory from API-key env vars or supported externally supplied bearer-token env vars; the provider layer does not run OAuth flows, refresh tokens, persist credentials, configure connectors, or grant runtime authority. Provider failures use `ModelProviderError` codes/categories instead of ad hoc parsing while keeping raw upstream error bodies and credentials out of durable evidence. Direct Anthropic Messages API OAuth is not implemented because the official API path uses `x-api-key`.
...
- Audit Security fixture evidence chains without mutating runtime state: `auditSecurityFixtureEvidence` verifies scan, poisoning, acknowledgement, decoy-only trial, and detector-only fixture artifacts against Ledger evidence and run manifests without reading raw content, executing subjects, repairing projections, or granting authority.
