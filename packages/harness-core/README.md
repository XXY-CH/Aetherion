...
- Audit Event Ledger `payload_ref` values for `agent.model.requested` events and future `agent.model.responded` events by resolving `artifact://agent/model-request/<id>` and `artifact://agent/model-response/<id>` and validating them against the request/response metadata schemas.
