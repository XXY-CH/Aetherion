...
- Create and validate Agent Runtime Invocation, Agent Model Request, and Agent Model Response metadata artifacts through local workspace helpers. Agent Model Request artifacts can now be derived from a bound runtime invocation as no-tools request metadata. These helpers persist only schema-valid ids, refs, hashes, gates, usage, and audit state; they do not call providers, persist raw prompt/model text, resolve credentials, request tools, issue leases, or grant authority.
- Audit Event Ledger `payload_ref` values for `agent.model.requested`, `agent.model.responded`, and `agent.response.audit.recorded` events by resolving `artifact://agent/model-request/<id>`, `artifact://agent/model-response/<id>`, and `artifact://agent/response-audit/<id>` and validating them against the request/response/audit metadata schemas.
