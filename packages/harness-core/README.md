...
- Validate the metadata-only Vault Reference and Model Provider Readiness contracts, rejecting raw secret, raw prompt/model payload, OAuth-flow, token-refresh, connector-grant, provider tool-call, and model-output authority claims in schema tests.
