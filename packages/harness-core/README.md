...
- Resolve no-tools live model providers behind the existing hash-only response boundary: `openai_responses`, `openai_chat_completions`, `anthropic`, and `gemini`. Provider credentials are read only in memory from API-key env vars or supported externally supplied bearer-token env vars; the provider layer does not run OAuth flows, persist credentials, configure connectors, or grant runtime authority. Direct Anthropic Messages API OAuth is not implemented because the official API path uses `x-api-key`.
