...
- 验证 Local Ingress Readiness、Local Ingress Rate Limit Reservation、Local Ingress Idempotency Reservation、metadata-only Vault Reference、Vault Policy Binding、Model Provider Readiness 与 Supervisor Lifecycle Readiness contract，并用 schema 测试拒绝 remote-surface ingress overclaim、unauthenticated authority、rate-limit authority/session/queue overclaim、idempotency authority reuse、raw rate-limit 或 idempotency key/intent persistence、late rate-limit enforcement、late duplicate detection、raw external payload storage、raw secret、secret resolution、egress、raw prompt/model payload、OAuth flow、token refresh、connector grant、provider tool call、model-output authority、production daemon、stale-lock repair、socket-auth authority、vault backend 和 supervisor lease authority 已实现的声明。
