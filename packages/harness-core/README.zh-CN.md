...
- 验证 Local Ingress Readiness、Local Ingress Rate Limit Reservation、Local Ingress Idempotency Reservation、Local Ingress Idempotency Completion、metadata-only Vault Reference、Vault Policy Binding、Model Provider Readiness、Supervisor Lifecycle Readiness、Supervisor Lifecycle Command、Supervisor Socket Auth Boundary、Ledger Integrity Extension Readiness 与 Adapter Gate Readiness contract，并用 schema 测试拒绝 remote-surface ingress overclaim、unauthenticated authority、rate-limit authority/session/queue overclaim、idempotency authority reuse、raw rate-limit 或 idempotency key/intent persistence、late rate-limit enforcement、late duplicate detection、mismatched 或 live idempotency replay claim、raw external payload storage、raw secret、secret resolution、egress、raw prompt/model payload、OAuth flow、token refresh、connector grant、provider tool call、model-output authority、production daemon、stale-lock repair、socket-auth authority、socket-token persistence、remote socket client、vault-backed socket-token storage、socket token 发 session/lease、vault backend、supervisor lifecycle side effect、unsupported-command authority、supervisor lease authority、runtime event signature、redaction tooling、automatic repair、projection authority、cloud notary、Ledger migration、真实 adapter 执行、adapter-issued lease、live side-effect replay、supervisor bypass、package execution 和 generated-package trust-root 已实现的声明。
