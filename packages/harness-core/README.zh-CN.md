...
- 验证 metadata-only Vault Reference、Model Provider Readiness 与 Supervisor Lifecycle Readiness contract，并用 schema 测试拒绝 raw secret、raw prompt/model payload、OAuth flow、token refresh、connector grant、provider tool call、model-output authority、production daemon、stale-lock repair、socket-auth authority、vault backend 和 supervisor lease authority 已实现的声明。
