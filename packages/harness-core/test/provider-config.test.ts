...
test("writeProviderConfig stores provider + model + key in JSON", async () => {
  ...
  writeProviderConfig(ws, { provider: "anthropic", model_ref: "claude-sonnet-4-20250514", api_key: "sk-ant-xxx" });
  const config = readProviderConfig(ws);
  assert.ok(config);
  assert.equal(config!.provider, "anthropic");
  assert.equal(config!.model_ref, "claude-sonnet-4-20250514");
  assert.equal(config!.api_key, "sk-ant-xxx");
});
...
test("provider-config.json has 0600 permissions", async () => {
  const ws = await makeWorkspace();
  writeProviderConfig(ws, { provider: "openai_responses", model_ref: "gpt-4o", api_key: "sk-xxx" });
...
test("redactApiKey masks the middle of the key", () => {
  const config = { provider: "anthropic", model_ref: "claude", api_key: "sk-ant-1234567890abcdef" };
