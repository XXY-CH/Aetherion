...
- Report supervisor status without appending Ledger events: workspace identity, transport, `daemon_running=false`, runtime paths, Ledger hash-chain validity, event count, head pointers, read-only runtime-lock fields, runtime-lock owner process status, and stale-lock detection without repair.
- The repository-level Supervisor Lifecycle Readiness contract treats this status/preflight evidence as observable readiness only; it does not claim production daemon start/stop, socket-auth lifecycle, stale-lock recovery, vault-backed supervisor secrets, or lease authority.
...
- Optionally bind the foreground Unix socket to one workspace with `--workspace-root <root>`, create `<root>/.aetherion/supervisor.lock` while the bound socket object is live, reject socket requests for other workspace roots or ids before dispatch, and expose the lock's claimed PID, transport, workspace id, and socket path through read-only status fields.
- Parse stdio RPC input with a dependency-free structured JSON object parser, fail closed on malformed JSON, duplicate keys, wrong-typed required string fields, and wrong-typed boolean approval fields, escape all JSON response control characters for traced read contents, reject legacy policy-only/read/write/replay RPCs that would bypass full Ledger lifecycle evidence, and return the actual operation lease id for approved write commits.
