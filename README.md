...
- `crates/supervisor/`: Rust Local Supervisor POC used by Ether by default: workspace init, versioned cross-author hash-chained JSONL event append/verification, deterministic local file policy, scoped leases, lease-gated reads, and traced write prepare/commit.
...
- `checkpoint`, `branch`, `rehearse`, and `approve-rehearsal` use checkpoint/branch registries with event id/hash pointers. File rehearsals write only to `.aetherion/sandboxes/<branch>/workspace/`, record original/proposed hashes and a reviewable diff, then approve through an independent promotion run whose Rust supervisor write-prepare/write-commit lifecycle records consent, fresh policy, lease, action, observation, and verification before exact-content promotion to the real workspace.
