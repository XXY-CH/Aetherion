...
- `prompt plan <run_id> --content <task>` is the first local Agent Orchestrator prompt assembly preview. It renders source-backed authority, tool, memory, taint, and response-contract sections for a task, but it does not call a model, execute tools, authorize actions, append Ledger events, or persist prompt artifacts.
