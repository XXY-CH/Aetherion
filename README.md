...
- `schemas/`: JSON Schemas for Event, Tool Request, Policy Decision, Scoped Lease, Action Record, Observation Record, Verification Record, Consent Record, Permission Policy, Memory Card, Memory Candidate, Memory Patch, Context Pack, Agent Runtime Invocation, Agent Model Request/Response metadata, Capability Capsule, Capability Package, Proactive Opportunity, Replay Record, and Migration Report.
...
`audit payload-refs` is a read-only Event Ledger artifact-reference audit. It scans Ledger events with `payload_ref`, resolves known local `artifact://` references such as Boundary Facts, Consent Records, Replay Records, Agent Runtime Invocation metadata, Agent Model Request/Response metadata, Capsule lifecycle snapshots, and generic Ether artifacts, and reports `resolved`, `missing`, `invalid_json`, or `unresolved` without writing artifacts, mutating registries, appending events, or treating artifacts as authority. For Boundary Facts, Consent Records, Replay Records, Agent Runtime Invocation metadata, Agent Model Request/Response metadata, and Capsule draft/test/publish/rollback snapshots, it also validates the parsed artifact JSON against the existing contract schema and reports `schema_valid`, `schema_invalid`, and `schema_not_checked` counters; generic artifacts remain path/JSON checks only until they have dedicated schemas.
...
- Agent Model Request/Response contracts define the next runtime evidence shape behind `agent.runtime.bound`. `prompt prepare-model-request <invocation_id>` reads a previously bound Agent Runtime Invocation artifact, writes schema-valid no-tools request metadata under `artifact://agent/model-request/<request_id>`, and records an independent supervisor-authored `agent.model.requested` event. The request artifact may record only provider/model refs, prompt/message hashes, context refs, no-tools mode, request payload hash, response expectations, and authority gates. It records `provider_called=false`, `network_call_attempted=false`, `tools_requested=false`, and no raw prompt/context payload. Response artifacts live under `artifact://agent/model-response/<response_id>` and may record only provider/model refs, response/output hashes, usage accounting, tool-call metadata, and response-audit requirement state. They must not persist raw prompt text, raw context prose, raw model output, raw provider payloads, raw secrets, or tool execution authority. No TUI command or provider loop creates response artifacts yet.
