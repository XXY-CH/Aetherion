...
- `prompt plan <run_id> --content <task>` is the first local Agent Orchestrator prompt assembly preview. It renders source-backed authority, run evidence, tool, memory, taint, response-contract, planner-checklist, and verification-checklist sections for a task from the Context Pack plus Ledger event envelopes, but it does not call a model, execute tools, authorize actions, append Ledger events, read raw payloads, or persist prompt artifacts.
