...
- Agent Model Request/Response contracts now define the next runtime evidence shape behind `agent.runtime.bound`. Request artifacts live under `artifact://agent/model-request/<request_id>` and may record only provider/model refs, prompt/message hashes, context refs, no-tools mode, request payload hash, response expectations, and authority gates. Response artifacts live under `artifact://agent/model-response/<response_id>` and may record only provider/model refs, response/output hashes, usage accounting, tool-call metadata, and response-audit requirement state. They must not persist raw prompt text, raw context prose, raw model output, raw provider payloads, raw secrets, or tool execution authority. No TUI command or provider loop creates these artifacts yet.
