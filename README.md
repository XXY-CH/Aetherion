...
For a read-only from-source onboarding preflight, run `npm run ether -- onboarding check --workspace .`. The report checks the local toolchain (`node`, `npm`, `git`, `rustc`, `cargo`, optional `cargo-audit`), repo scripts, lockfiles, CI gates, governance files, bilingual docs, workspace runtime state, and onboarding doc links, then prints next-step commands. It does not install dependencies, run the verification suite, initialize `.aetherion`, start or repair a daemon, write artifacts, call providers, query remote CI, or enable deferred GUI/IM/browser/MCP/OAuth/cloud/package-code surfaces.
