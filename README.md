...
For a read-only local release-evidence snapshot, run `npm run ether -- release evidence --workspace .`. The report combines git head/dirty state, configured CI gates, Node 24 action-runtime evidence, Ubuntu/macOS smoke configuration, dependency lockfile evidence, governance and bilingual-doc checks, `doctor` summary, `security audit` summary, workspace runtime/Ledger state, source-document links, and explicit remaining release gaps. It is local/configured evidence only: it does not query remote CI, package or sign artifacts, publish releases, deploy docs, initialize workspaces, write `.aetherion` artifacts, call providers, issue leases, or enable deferred GUI/IM/browser/MCP/OAuth/cloud/package-code surfaces.
